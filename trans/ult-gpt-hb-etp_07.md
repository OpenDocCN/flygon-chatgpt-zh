...
# [介绍 LangChain：释放大型语言模型的潜力](toc.xhtml#s429a)
...
有效的结构化提示是释放 GPT 模型巨大潜力的关键。为了适应不同的用例，我们可以采用一系列提示模式，例如指令提示、指令序列提示、查询提示、请求提示、合作提示或其他在之前的*[第 5 章，高级提示工程技术](c05.xhtml)*中提到的特殊提示。
...
使用独立的 LLM 的主要挑战之一是它们只能访问它们专门训练过的数据。为了克服这一限制并使它们能够使用最新信息，我们必须将它们连接到其他数据源。幸运的是，LangChain 提供了有效的组件，可以轻松地与各种数据源进行交互（详细探讨了用于访问企业特定数据源的 A2-A4 架构模式，请参阅*[第 4 章，由 GPT 模型启用的架构模式](c04.xhtml)*）。
...
另一个常用的链是简单顺序链。在这种类型中，初始调用后会对同一个或不同的语言模型进行一系列调用。当一个调用的输出需要作为另一个调用的输入时，这是特别有用的（参见*[第 4 章，由 GPT 模型实现的架构模式](c04.xhtml)*中的 C2、D1 和 D2 模式）。
...
# [嵌入民事案件并将其存储在 Pinecone 向量存储中](toc.xhtml#s451a)
...
# [示例 2：自动化专家的内部知识 QA-在特定控制器手册上聊天](toc.xhtml#s453a)
...
# [将手册分块并加载到 Pinecone 向量存储器](toc.xhtml#s454a)
...
# [示例 3：使用 LangChain 代理进行市场研究](toc.xhtml#s457a)
