![图片](img/100-chatgpt-prompts-for-beginner-Image00000.jpg)
...
为了解决这个问题，请使用`let`而不是`var`，或者创建一个 IIFE（立即调用的函数表达式）来捕获`i`的当前值。
...
在我们开始编码之前，请确保您的系统已安装 Python。您可以从[官方 Python 网站](https://www.python.org/downloads/)下载它。此外，PyCharm 或 VSCode 等集成开发环境（IDE）对于编写和测试脚本可能很有帮助。
